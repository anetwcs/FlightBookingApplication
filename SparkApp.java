...
import scala.Tuple2;
// An Hong Nguyen
//Source idea for QC: https://heather.miller.am/teaching/cs212/slides/week19b.pdf
...
//                .sortBy(t -> t.getString(0), true, 1);
    ...
    public static JavaRDD<Row> QA(SparkSession spark, String dataFile) {
        Dataset<Row> d = spark.read().parquet(dataFile);
        Dataset<Row> rowA = d.where(d.col("origincityname").equalTo("Seattle, WA")).select(d.col("destcityname")).distinct();
        return rowA.javaRDD();
    }
    ...
    public static JavaRDD<Row> QB(SparkSession spark, String dataFile) {
        Dataset<Row> d = spark.read().parquet(dataFile);
        JavaRDD<Row> rdd = d.javaRDD();
        JavaPairRDD<Tuple2<Integer, String>, Integer> pair1 = rdd.filter(t -> t.getInt(CANCELLED) != 1).mapToPair(
                t-> new Tuple2<>(new Tuple2<>(t.getInt(MONTH), t.getString(ORIGIN_CITY_NAME)), 1));
        JavaRDD<Row> rddB = pair1.reduceByKey((v1,v2) -> v1 + v2).map(t->RowFactory.create(t._1()._2(), t._1()._1(), t._2()));
    ...
    public static JavaRDD<Row> QC(SparkSession spark, String dataFile) {
        Dataset<Row> d = spark.read().parquet(dataFile);
        JavaRDD<Row> rdd = d.javaRDD();
        JavaPairRDD<String, Tuple2<Integer, Integer>> pair2 = rdd.filter(t -> t.get(DEP_DELAY) != null)
                .filter(t -> t.getInt(CANCELLED) != 1).mapToPair(t -> new Tuple2<>(t.getString(ORIGIN_CITY_NAME),
                        new Tuple2<>(t.getInt(DEP_DELAY), 1)));
        JavaRDD<Row> rddC = pair2.reduceByKey((v1, v2) -> new Tuple2<>(v1._1() + v2._1(), v1._2() + v2._2())).map(
                t -> RowFactory.create(t._1(), (double)t._2()._1() / t._2()._2()));
        return rddC;
    }
}
